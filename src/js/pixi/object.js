...
function createGradientCircle(x, y, radius, colorFrom, colorTo) {
  ...
  const gradient = ctx.createLinearGradient(0, 0, radius * 2, 0);

  // Colorが数字の場合に変換する。
  let colorFromTrue = typeof colorFrom === "number"
    ? '#' + ('00000' + (colorFrom | 0).toString(16)).substr(-6)
    : colorFrom;
  let colorToTrue = typeof colorTo === "number"
    ? '#' + ('00000' + (colorTo | 0).toString(16)).substr(-6)
    : colorTo;
